"""Criando um projeto e lista de tarefas usando flet"""
...
class TodoApp(UserControl):
    def build(self):
        ...
        self.itens_left = Text('0 tarefas adicionadas')
    ...
    def update(self):
        ...
        for task in self.tasks.controls:
            task.visible = (
                status == 'Todas'
                or (status == 'Ativas' and task.completed is False)
                or (status == 'Completas' and task.completed)
            )
            if not task.completed:
                count += 1
        self.itens_left.value = f'{count} tarefa(s) adicionada(s)'
        super().update()
